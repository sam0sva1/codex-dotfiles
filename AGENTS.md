...
This file is the Codex adaptation of `~/.pi/agent/CLAUDE.md`. Keep it as close as possible to that source; only the compatibility notes below should differ across Codex, Pi, and Claude Code.

## Codex Compatibility
- References like `/brainstorm` mean the `$brainstorm` skill in Codex.
- References like `/go-plan`, `/plan-check`, `/execute-plan`, `/tdd`, `/self-review`, `/task-commit`, `/preflight`, `/api-contract-first`, and `/cross-service-changes` mean same-named `$...` skills.
- References like `@architect`, `@critic`, `@debugger`, and `@explorer` mean same-named `$...` skills in Codex.
- References like `@planner` mean the `$high-level-planner` skill in Codex.
- For the maintenance procedure for syncing Codex from `~/.claude`, see `PORTING_FROM_CLAUDE.md`.
- When updating this file, prefer wording that can be copied back to Pi or Claude Code with minimal rewriting.
